...
export async function createWizardMcpServer(
  ...
  server.registerTool(
    MCP_TOOLS.proposeTrackingPlan,
    {
      ...
      description:
        "Validate and stamp identities, custom events, properties, rationale, and provenance generated by the MCP client model from explicit business context and normalized project inspection. Revenue events require explicit revenue context and authoritative server-capable events. Set baseline: true (without other inputs) for a deterministic page-view-only plan that needs no AI proposal. Never modifies files.",
      inputSchema: {
        project_path: projectPathInputSchema.optional(),
        baseline: z.boolean().optional(),
        business_context: businessContextSchema.optional(),
        ai_proposal: aiTrackingProposalSchema.optional(),
      },
      outputSchema: trackingPlanSchema.shape,
      annotations: readOnlyAnnotations,
    },
    async ({ project_path, baseline, business_context, ai_proposal }) => {
      try {
        const projectRoot = await resolveProjectPath(root, project_path);
        const inspection = await inspectProject(projectRoot);
        if (baseline === true) {
          if (business_context !== undefined || ai_proposal !== undefined) {
            throw new Error(
              "Baseline planning does not accept AI planning inputs",
            );
          }
        ...
        if (business_context === undefined || ai_proposal === undefined) {
          throw new Error(
            "AI planning requires business_context and ai_proposal (or set baseline: true)",
          );
        }
  ...
  server.registerTool(
    MCP_TOOLS.generateSetupPlan,
    {
      ...
      inputSchema: {
        ...
        ai_instrumentation: aiInstrumentationProposalSchema.optional(),
      },
  ...
  server.registerTool(
    MCP_TOOLS.doctor,
    {
      ...
      description:
        "Run read-only local diagnostics: Node.js version, project root, framework support, package manager, private state directory, and optional tracking-host reachability. Returns normalized statuses and bounded detail strings only.",
      inputSchema: {
        project_path: projectPathInputSchema.optional(),
        tracking_host: z.url().max(2_000).optional(),
      },
      outputSchema: doctorReportSchema.shape,
      annotations: readOnlyAnnotations,
    },
    async ({ project_path, tracking_host }) => {
      try {
        const projectRoot = await resolveProjectPath(root, project_path);
        const result = await runDoctor({
          projectRoot,
          ...(tracking_host ? { trackingHost: tracking_host } : {}),
        });
        return {
          content: [{ type: "text" as const, text: JSON.stringify(result) }],
          structuredContent: result,
